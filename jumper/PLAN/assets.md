...
**Missing for v1:** player/character sprites, music, SFX.
...
public/assets/sprites/   (later)
public/assets/audio/     (later)
```

The full pack library lives in `assets-source/Environments/` (not under `public/`, so Vite does not copy it into `dist`). Do not ship PSD, Aseprite, or preview images in published level folders.
